...
[Everthine](https://github.com/aveluneverse/Everthine) keeps its companion in Telegram, uses the Claude Code CLI as the engine, and keeps memory on disk. Its `bot.py` has a seam, `produce_reply()`, that turns one message into one reply. `companion_server.py` exposes that seam as `POST /reply`, the route the [http-bridge](../http-bridge/README.md) talks to. Everthine's code is not modified and none of its files are copied: the script imports Everthine in place and calls `produce_reply()`, so memory and session files are read and written by Everthine itself, exactly as they are from Telegram.
...
The adapter listens on `127.0.0.1:8401` by default; `--port` changes the port, and `--reply` must then use the same one (`--host` only changes the address it listens on). `--reply-timeout 180` gives the Claude CLI the same three minutes the checker below allows; without it the bridge gives up at 120 seconds.
...
- "New conversation" from the MENU produces no reply: Everthine keeps one continuous conversation and has no reset, and the bridge shows a Chat Log notice saying so.
