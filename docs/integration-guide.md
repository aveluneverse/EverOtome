...
Three more shapes can arrive on the same socket, all JSON objects with a `text` field: `{ "text": "...", "photos": [...] }` after a photo upload, `{ "call": true, "text": "..." }` during a phone call, and the bare strings `/new`, `/status` and `/cg <id>` from menu items. If you do nothing else, read `text` out of the JSON ones and treat the slash strings as you like; ignoring them is fine.
...
**B. Add the WebSocket to the companion's own process.** Right when the companion is a Python or Node program you can edit. Both examples below serve `engine/` and answer `/ws`; replace the one function that fakes the reply with a call into the companion. Both were run against this repository on 2026-08-28.
...
**C. Reverse proxy.** When the companion already runs a web server on another port and you would rather not touch it, put a proxy in front: static files from `engine/`, `/ws` and `/api/` forwarded to the companion. Two configurations that were run against this repository (Caddy and nginx) are in the [contract's deployment section](backend-contract.md#deployment-one-origin). The companion still has to answer `/ws` with the frame above.

## Five traps

1. **Same origin.** The shell builds the socket address from the page's own host plus the path in `wsEndpoint`. A page served from port 8400 cannot talk to a socket on port 5000. Either the companion's process serves `engine/` itself, or one proxy (or the bridge) fronts both.
